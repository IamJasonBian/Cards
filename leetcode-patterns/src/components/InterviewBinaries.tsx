  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="space-y-6">
        <section>
          ...
          <div className="space-y-4">
            {coderpadProblems.map((problem, idx) => (
              <div
                key={idx}
                className="bg-slate-900/85 sm:bg-slate-900/70 backdrop-blur-2xl border border-white/10 rounded-none p-6 hover:shadow-md transition-shadow"
              >
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {problem.topics.map((topic) => (
                    <span
                      key={topic}
                      className="px-2.5 py-1 bg-cyan-500/20 text-cyan-300 rounded-none text-xs font-medium"
                    >
                      {topic}
                    </span>
                  ))}
                ...
                <div className="border-t border-white/5 pt-4">
                  ...
                  <ul className="grid grid-cols-2 gap-2">
                    {problem.parts.map((part, partIdx) => (
                      ...
                      >
                        <span className="w-5 h-5 rounded-none bg-slate-800/90 sm:bg-slate-800/70 text-slate-300 flex items-center justify-center text-xs font-medium">
                          {partIdx + 1}
                        </span>
                        {part}
                      </li>
        ...
        <section className="bg-cyan-500/10 border border-cyan-500/30 rounded-none p-6">
