...
export function PatternCard({ pattern }: { pattern: Pattern }) {
  ...
  return (
    <div className="bg-slate-900/20 backdrop-blur-2xl rounded-2xl shadow-sm border border-white/10 hover:shadow-md transition-shadow">
      <button
        onClick={() => setOpen(!open)}
        className="w-full p-5 flex items-center justify-between text-left cursor-pointer"
      >
        <div className="flex items-center gap-4">
          <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-slate-800/15 text-slate-400 font-mono text-sm font-bold">
            {pattern.id}
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {pattern.problems.map((p) => (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 px-2 py-1 bg-slate-800/15 border border-white/10 rounded text-xs text-slate-300 hover:bg-white/5 hover:border-cyan-400/30 hover:text-cyan-300 transition-colors"
                >
                  <span
