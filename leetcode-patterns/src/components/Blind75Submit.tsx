...
export function Blind75Submit() {
  ...
  if (problemError) {
    return (
      <div className="max-w-2xl rounded-2xl border border-rose-400/30 bg-rose-500/15 backdrop-blur-2xl p-4 text-sm text-rose-200">
    ...
    <div className="max-w-7xl">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-1">
          ...
          <span className="px-2 py-0.5 rounded-full text-[11px] font-medium bg-slate-800/15 text-slate-300 border border-white/10">
            {problem.category}
          </span>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {sortedManifest.length > 1 && (
            <select
              ...
              className="ml-auto text-sm border border-white/10 rounded-2xl px-2 py-1 bg-slate-900/20 backdrop-blur-2xl text-slate-100 focus:ring-2 focus:ring-cyan-400/40 focus:outline-none"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="bg-slate-900/20 backdrop-blur-2xl rounded-2xl border border-white/10 p-5">
          ...
          <div className="space-y-3">
            {problem.visibleExamples.map((ex, i) => (
              <div key={i} className="rounded-2xl border border-white/10 bg-slate-800/15 p-3 text-xs">
        ...
        <section className="space-y-4">
          <div className="bg-slate-900/20 backdrop-blur-2xl rounded-2xl border border-white/10 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2 border-b border-white/10 bg-slate-800/15">
            ...
            <textarea
              ...
              spellCheck={false}
              className="w-full font-mono text-sm bg-slate-950/40 text-slate-100 p-4 outline-none resize-y leading-relaxed focus:ring-2 focus:ring-cyan-400/40"
              style={{ minHeight: 360, tabSize: 4 }}
            />
            <div className="flex items-center justify-between px-4 py-3 bg-slate-800/15 border-t border-white/10">
              <span className="text-[11px] text-slate-400">Tab indents · ⌘/Ctrl + Enter to submit</span>
              <button
                onClick={submit}
                disabled={running}
                className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-cyan-500 text-slate-950 text-sm font-medium hover:bg-cyan-400 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer transition-colors"
              >
...
function ResultsPanel({ result }: { result: RunResult }) {
  ...
  return (
    <div className="bg-slate-900/20 backdrop-blur-2xl rounded-2xl border border-white/10 p-5">
      ...
      {result.fatal && (
        <pre className="bg-rose-500/15 border border-rose-400/30 text-rose-200 text-xs font-mono p-3 rounded-2xl whitespace-pre-wrap mb-3">
          {result.fatal}
        </pre>
      )}
...
function CaseRow({ c }: { c: CaseResult }) {
  return (
    <div
      className={`rounded-2xl border p-3 text-xs ${
        c.passed ? "bg-emerald-500/15 border-emerald-400/30" : "bg-rose-500/15 border-rose-400/30"
      }`}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
