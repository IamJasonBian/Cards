...
function difficultyClass(d: string): string {
  if (d === "Easy") return "bg-emerald-500/10 text-emerald-300 border-emerald-500/30";
  if (d === "Medium") return "bg-amber-500/10 text-amber-300 border-amber-500/30";
...
export function Blind75Submit() {
  ...
  if (problemError) {
    return (
      <div className="max-w-2xl rounded-none border border-rose-500/30 bg-rose-500/10 backdrop-blur-2xl p-4 text-sm text-rose-300">
  ...
  if (!problem) {
    return (
      <div className="max-w-2xl text-sm text-slate-400 flex items-center gap-2 py-12">
    ...
    <div className="max-w-7xl">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-1">
          ...
          <span className="px-2 py-0.5 rounded-none text-[11px] font-medium bg-slate-800/90 sm:bg-slate-800/70 text-slate-300 border border-white/10">
            {problem.category}
          </span>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="text-3xl font-bold text-slate-100">{problem.title}</h1>
          <a
            href={problem.url}
            target="_blank"
            rel="noreferrer"
          ...
          {sortedManifest.length > 1 && (
            <select
              ...
              className="ml-auto text-sm border border-white/10 rounded-none px-2 py-1 bg-slate-900/85 sm:bg-slate-900/70 backdrop-blur-2xl text-slate-100 focus:ring-2 focus:ring-cyan-400/40 focus:outline-none"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="bg-slate-900/85 sm:bg-slate-900/70 backdrop-blur-2xl rounded-none border border-white/10 p-5">
          ...
          <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wide mt-4 mb-2 flex items-center gap-1">
            <Eye size={12} />
            Examples
          </h3>
          <div className="space-y-3">
            {problem.visibleExamples.map((ex, i) => (
              <div key={i} className="rounded-none border border-white/10 bg-slate-800/90 sm:bg-slate-800/70 p-3 text-xs">
                <div className="font-mono text-slate-300">
                  <span className="text-slate-400">Input:</span> {formatInputCompact(ex.input, 200)}
                </div>
                <div className="font-mono text-slate-300">
                  <span className="text-slate-400">Output:</span> {formatValue(ex.output, 200)}
                </div>
        ...
        <section className="space-y-4">
          <div className="bg-slate-900/85 sm:bg-slate-900/70 backdrop-blur-2xl rounded-none border border-white/10 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2 border-b border-white/10 bg-slate-800/90 sm:bg-slate-800/70">
            ...
            <textarea
              ...
              spellCheck={false}
              className="w-full font-mono text-sm bg-slate-800/90 text-slate-100 p-4 outline-none resize-y leading-relaxed focus:ring-2 focus:ring-cyan-400/40"
              style={{ minHeight: 360, tabSize: 4 }}
            />
            <div className="flex items-center justify-between px-4 py-3 bg-slate-800/90 sm:bg-slate-800/70 border-t border-white/10">
...
function ResultsPanel({ result }: { result: RunResult }) {
  ...
  return (
    <div className="bg-slate-900/85 sm:bg-slate-900/70 backdrop-blur-2xl rounded-none border border-white/10 p-5">
      ...
      {result.fatal && (
        <pre className="bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs font-mono p-3 rounded-none whitespace-pre-wrap mb-3">
          {result.fatal}
        </pre>
      )}
      ...
      {result.hiddenTotal > 0 && (
        <div className="space-y-2">
          <div className="text-[11px] uppercase tracking-wide text-slate-400 font-semibold flex items-center gap-1">
            <EyeOff size={11} />
            Hidden cases
          </div>
          <div
          ...
          >
            ...
            {firstFailingHidden && (
              <div className="mt-2 pt-2 border-t border-white/10 font-mono text-[11px] text-slate-300 grid gap-0.5">
                <div>
                  <span className="text-slate-400">input:&nbsp;&nbsp;&nbsp;</span>
                  {formatInputCompact(firstFailingHidden.input, 60)}
                </div>
                <div>
                  <span className="text-slate-400">expected:</span> {formatValue(firstFailingHidden.expected, 60)}
                </div>
                <div>
                  <span className="text-slate-400">actual:&nbsp;&nbsp;</span> {formatValue(firstFailingHidden.actual, 60)}
                </div>
                {firstFailingHidden.error && (
                  <div className="text-rose-300 mt-1 whitespace-pre-wrap">{firstFailingHidden.error}</div>
                )}
              </div>
            )}
          </div>
...
function CaseRow({ c }: { c: CaseResult }) {
  return (
    <div
      className={`rounded-none border p-3 text-xs ${
        c.passed ? "bg-emerald-500/10 border-emerald-500/30" : "bg-rose-500/10 border-rose-500/30"
      }`}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
      ...
      {!c.passed && (
        <div className="font-mono text-[11px] text-slate-300 grid grid-cols-1 gap-0.5 ml-5 mt-1">
          {c.input && (
            <div>
              <span className="text-slate-400">input:&nbsp;&nbsp;&nbsp;</span>
          ...
          <div>
            <span className="text-slate-400">expected:</span> {formatValue(c.expected, 200)}
          </div>
          <div>
            <span className="text-slate-400">actual:&nbsp;&nbsp;</span> {formatValue(c.actual, 200)}
          </div>
          {c.error && <div className="text-rose-300 mt-1 whitespace-pre-wrap">{c.error}</div>}
        </div>
      )}
    </div>
  );
