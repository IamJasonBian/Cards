  ...
  return (
    <section className="mx-auto w-full max-w-5xl px-4 sm:px-6 py-16 sm:py-24">
      <div className="mb-6 flex items-baseline justify-between">
        <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 [text-shadow:_0_2px_18px_rgb(255_255_255_/_70%)]">
          Popular lists
        </h2>
        <button
          type="button"
          onClick={() => onNavigate("popular-lists")}
          className="text-sm font-medium text-cyan-700 hover:text-cyan-800 inline-flex items-center gap-1 cursor-pointer [text-shadow:_0_1px_12px_rgb(255_255_255_/_80%)]"
        >
          View all
          <ArrowRight size={14} />
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-slate-900/10">
        {featuredLists.map((list) => (
          <button
            ...
            type="button"
            onClick={() => onNavigate("popular-lists")}
            className="bg-white/60 backdrop-blur-sm hover:bg-white/80 p-6 text-left transition-colors cursor-pointer flex flex-col gap-3"
          >
