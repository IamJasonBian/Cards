...
export function PatternSidebar({
  ...
  return (
    <aside className="hidden md:block w-64 shrink-0 sticky top-16 h-[calc(100vh-64px)] overflow-y-auto bg-slate-900/20 backdrop-blur-2xl border-r border-white/10">
      <div className="p-4">
          ...
          {patterns.map((p) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-0.5 ml-7">
                <span
                  className={`px-1.5 py-0 rounded-full text-[10px] font-medium ${tagColors[p.tag] || "bg-slate-800/15 text-slate-300"}`}
                >
                  {p.tag}
                </span>
